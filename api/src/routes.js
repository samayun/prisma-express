...
const swaggerModule = require('./app/swagger/routes');

const routes = [
  {
    name: 'Dynamic Modules',
    type: 'module',
    prefix: config.api.apiRoutePrefix,
    handler: loadModules,
  },

  {
    ...
    handler: (req, res) => {
      res.json({
        status: 'All is well 🐦',
        site: 'prisma.js monolithic framework',
        docs: `http://${req.headers.host}${config.api.swaggerRoutePrefix}`,
        author: {
  ...
  {
    name: 'Swagger Route',
    path: config.api.swaggerRoutePrefix,
